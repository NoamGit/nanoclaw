...
4. Add the product via `$.ajax` — **do NOT use `createNewProductSearch`/`submitWishList()`** (unreliable; the submitList event has no parent handler on the edit page):
```javascript
// Quantity units — check sellingMethod.code in the product data:
// BY_WEIGHT (fruits/veg/fish/deli): quantity = kg as FLOAT (e.g. 2.0 for 2kg, 1.5 for 1.5kg)
//   NEVER pass grams — 2000 means 2000kg (will be capped at maxOrderWeight=100kg)!
// BY_PACKAGE: quantity = number of packages. Check unitDescription for kg/package.
// BY_UNIT (default): quantity = integer count.
$.ajax({
  url: ACC.config.encodedContextPath + '/wishlist/editWishlist',
  type: 'POST',
  data: JSON.stringify({
    name: 'LIST_NAME',    // wishlist name string (NOT the numeric ID)
    newName: 'LIST_NAME',
    imageCode: '',
    params: [{productCode: 'P_XXXXX', unitOfMeasure: 'BY_UNIT', quantity: 1, comment: '', remove: false}]
  }),
  contentType: 'application/json',
  success: function(d){ window._addResult = 'OK:' + d.entries.length; },
  error: function(xhr){ window._addResult = 'ERR:' + xhr.status + ' ' + xhr.responseText.substring(0,200); }
});
```
To remove an item, use the same call with `remove: true`.

5. Wait 3 seconds, then verify entry count:
```bash
browser-use --session superme eval "$.ajax({url: ACC.config.encodedContextPath+'/wishlist/getWishlist?listId=LIST_ID', success:function(d){window._count=d.entries.length;}});"
```

6. Confirm to user: "Added [product name] to your Superme List. [N] items total."
...
4. Consolidate products, deduplicate by code. Exclude delivery items ("משלוח", "דמי"). For each product:
   - Check `sellingMethod.code` on the product. Values: `BY_WEIGHT`, `BY_PACKAGE`, `BY_UNIT`.
   - **BY_WEIGHT items (fruits, veg, fish, deli): the order `quantity` is in grams. Divide by 1000 to get kg before averaging.** Example: order has banana qty=2000 → 2.0 kg. Average in kg, keep 1 decimal place, minimum 0.5.
   - BY_UNIT / BY_PACKAGE items: quantity is already the right unit. Average normally, round to nearest integer, minimum 1.
   - Example: milk (BY_UNIT) with qtys 3,2,4 → avg=3. Banana (BY_WEIGHT) with qtys 2000,1500,2000g → avg=1833g → 1.8kg.
...
7. Navigate to the wishlist edit page and add all products via `$.ajax` in a single batch — **do NOT use the Vue component** (`createNewProductSearch`/`submitWishList()` is unreliable; the submitList event has no parent handler on this page):
```bash
browser-use --session superme open "https://www.shufersal.co.il/online/he/my-account/personal-area/wish-list-2-cart/LIST_ID"
```
Then send the full batch in one eval:
```javascript
// CRITICAL quantity units:
// BY_WEIGHT: quantity = kg as float (e.g. 2.0 for 2kg). NEVER pass grams (2000 = 2000kg)!
// BY_PACKAGE: quantity = number of packages (integer)
// BY_UNIT: quantity = integer count
var params = [ /* {productCode, unitOfMeasure, quantity, comment: '', remove: false} */ ];
$.ajax({
  url: ACC.config.encodedContextPath + '/wishlist/editWishlist',
  type: 'POST',
  data: JSON.stringify({name: 'LIST_NAME', newName: 'LIST_NAME', imageCode: '', params: params}),
  contentType: 'application/json',
  success: function(d){ window._mgResult = 'OK:' + d.entries.length; },
  error: function(xhr){ window._mgResult = 'ERR:' + xhr.status + ' ' + xhr.responseText.substring(0,200); }
});
```
Wait 3s then check `window._mgResult`.
